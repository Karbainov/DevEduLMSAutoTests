...
Scenario: Create and edit new group
	...
	| Ложкин    | Андрей     | string     | ilya@tutor.com    | dron     | password | SaintPetersburg | 03.03.1994 | string        | 89995554433 | Tutor   |
	When manager create new group from browser
	| Name | Course | Teachers | Tutors |
	| TG1 | Backend C# | pip sum, pop sam | van dam |
	And manager add students to group "TG1"
	Then users should see their group
	And manager should see actual group
	When manager edit group ""
	And manager delete students from group ""
	| Ilya1 Baikov | Ilya2 Baikov | Ilya3 Baikov |
	Then users should see their group
	And manager should see actual group
