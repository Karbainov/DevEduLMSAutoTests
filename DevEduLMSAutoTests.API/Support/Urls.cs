namespace DevEduLMSAutoTests.API.Support
{
    public class Urls
    {
        public const string Host = $"https://piter-education.ru:7070";
        public const string Register = $"{Host}/register";
        public const string Sign_in = $"{Host}/sign-in";
        public const string Users = $"{Host}/api/Users";
        public const string Comments = $"{Host}/api/Comments";
        public const string Courses = $"{Host}/api/Courses";
        public const string Materials = $"{Host}/api/Materials";
        public const string Notifications = $"{Host}/api/Notifications";
        public const string Payments = $"{Host}/api/Payments";
        public const string Ratings = $"{Host}/api/Ratings";
        public const string StudentHomeworks = $"{Host}/api/student-homeworks";
        public const string Groups = $"{Host}/api/Groups";
        public const string Tasks = $"{Host}/api/Tasks";
        public const string Homeworks = $"{Host}/api/Homeworks";
        public const string Topics = $"{Host}/api/Topics";
        public const string Lessons = $"{Host}/api/Lessons";
        public const string Courses = $"{Host}/api/Courses";

    }
}